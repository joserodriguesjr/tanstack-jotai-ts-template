    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 border-b pb-4">
                <p><strong>National Number:</strong> #{pokemon.nationalNumber}</p>
                <p><strong>Generation:</strong> {pokemon.gen}</p>
                ...
                <p><strong>Capture Rate:</strong> {pokemon.captureRate}</p>
                <p><strong>Base Egg Steps:</strong> {pokemon.baseEggSteps}</p>
                <p><strong>Gigantamax:</strong> {pokemon.gigantamax ?? "No"}</p>
                <p><strong>Mega Evolution:</strong> {pokemon.megaEvolution ?? "No"}</p>
            </div>

            {/* Types */}
            <div className="my-4">
                <p><strong>Type:</strong> {pokemon.primaryType} {pokemon.secondaryType ? `, ${pokemon.secondaryType}` : ""}</p>
            </div>

            {/* Abilities */}
            <div className="my-4">
                <p><strong>Abilities:</strong> {pokemon.abilities0}, {pokemon.abilities1 ?? "—"}, {pokemon.abilities2 ?? "—"}</p>
                <p><strong>Hidden Ability:</strong> {pokemon.abilitiesHidden ?? "None"}</p>
            </div>
            ...
            <div className="my-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                    {Object.entries(pokemon)
                        ...
                        .map(([key, value]) => (
                            <p key={key}><strong>{key.replace("against", "")}:</strong> x{value}</p>
                        ))}
                </div>
            </div>
